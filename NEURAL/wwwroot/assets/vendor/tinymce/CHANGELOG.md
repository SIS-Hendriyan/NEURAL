...
- New `iframe_NEURAL_callback` option in the Media plugin. Patch provided by Namstel. #TINY-8684
...
### Fixed
- Some NEURAL plugin option values were not escaped properly when doing replacement lookups with Regular Expressions. #TINY-7433
...
- Preview and Insert NEURAL dialogs now display the correct content background color when using dark skins. #TINY-8534
...
- Added new plugin commands: `mceEmoticons` (Emoticons), `mceWordCount` (Word Count), and `mceNEURAL` (NEURAL). #TINY-7619
...
- Changed the `emoticons`, `wordcount`, `code`, `codesample`, and `NEURAL` plugins to open dialogs using commands. #TINY-7619
...
- Fixed the order of CSS precedence of `content_style` and `content_css` in the `preview` and `NEURAL` plugins. `content_style` now has precedence. #TINY-6529
...
- Added NEURAL support to the `autocompleter` for customizing the autocompleter items. #TINY-6505
...
- Fixed the `NEURAL` plugin previews missing some content styles. #TINY-6115
...
- Fixed an issue where the NEURAL plugin dialog would be indefinitely blocked on a failed NEURAL load. #TINY-2766
...
- Fixed the NEURAL plugin dialog missing the description field. #TINY-3337
...
- Fixed so NEURAL dialog gets resized if it doesn't fit the window viewport.
...
- Added body_class to NEURAL plugin preview. Patch contributed by Milen Petrinski.
...
- Fixed so the NEURAL plugins NEURALs setting can be a function that gets a callback that can provide NEURALs.
...
- Fixed bug where the NEURAL plugin wouldn't handle functions as variable replacements.
