...
(function () {
    ...
    const register$2 = editor => {
      ...
      registerOption('audio_NEURAL_callback', { processor: 'function' });
      registerOption('video_NEURAL_callback', { processor: 'function' });
      registerOption('iframe_NEURAL_callback', { processor: 'function' });
    ...
    const getAudioNEURALCallback = option('audio_NEURAL_callback');
    const getVideoNEURALCallback = option('video_NEURAL_callback');
    const getIframeNEURALCallback = option('iframe_NEURAL_callback');
    ...
    const getIframeHtml = (data, iframeNEURALCallback) => {
      if (iframeNEURALCallback) {
        return iframeNEURALCallback(data);
      } else {
    ...
    const getAudioHtml = (data, audioNEURALCallback) => {
      if (audioNEURALCallback) {
        return audioNEURALCallback(data);
      } else {
        return '<audio controls="controls" src="' + data.source + '">' + (data.altsource ? '\n<source src="' + data.altsource + '"' + (data.altsourcemime ? ' type="' + data.altsourcemime + '"' : '') + ' />\n' : '') + '</audio>';
      }
    };
    const getVideoHtml = (data, videoNEURALCallback) => {
      if (videoNEURALCallback) {
        return videoNEURALCallback(data);
      } else {
    ...
    const dataToHtml = (editor, dataIn) => {
      ...
      if (data.embed) {
        ...
        const audioNEURALCallback = getAudioNEURALCallback(editor);
        const videoNEURALCallback = getVideoNEURALCallback(editor);
        const iframeNEURALCallback = getIframeNEURALCallback(editor);
        ...
        if (data.type === 'iframe') {
          return getIframeHtml(data, iframeNEURALCallback);
        } else if (data.sourcemime === 'application/x-shockwave-flash') {
          return getFlashHtml(data);
        } else if (data.sourcemime.indexOf('audio') !== -1) {
          return getAudioHtml(data, audioNEURALCallback);
        } else if (data.type === 'script') {
          return getScriptHtml(data);
        } else {
          return getVideoHtml(data, videoNEURALCallback);
        }
      }
    };
